...
class PathAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (!paths[position].matches("^[a-zA-Z0-9]+$".toRegex())) {
            ...
            holder.itemView.status.text = "Каталог готов к отправке"
            holder.itemView.status.setTextColor(
                ContextCompat.getColor(
                    context,
                    R.color.colorAccent
            ...
            runBlocking {
                GlobalScope.launch {
                    withContext(Dispatchers.Main) {
                        holder.itemView.status.text = "Начинается отправка"
                    ...
                    withContext(Dispatchers.Main) {
                        holder.itemView.status.text = "Каталог отправлен успешно"
                        val sharedAxis = MaterialSharedAxis(MaterialSharedAxis.X, true)
